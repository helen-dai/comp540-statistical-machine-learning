...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        J = (np.matmul(d.T, d) + reg*np.matmul(theta[1:].T, theta[1:])) / (2*num_examples)
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return J
    ...
    def grad_loss(self,*args):                                                                          
        ...
        ###########################################################################
        d = np.matmul(X, theta) - y
        grad[0] = np.matmul(d, X[:, 0]) / num_examples
        grad[1:] = (np.matmul(d, X[:, 1:]) + reg*theta[1:]) / num_examples
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return grad
...
class LassoLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        ###########################################################################
        d = np.matmul(X, theta) - y
        J = np.matmul(d.T, d) / (2*num_examples) + reg*np.sum(np.abs(theta[1:])) / num_examples
    ...
    def grad_loss(self,*args):                                                                          
        ...
        d = np.matmul(X, theta) - y
        grad[0] = np.matmul(d, X[:, 0]) / num_examples
        grad[1:] = (np.matmul(d, X[:, 1:]) + reg*np.sign(theta[1:])) / num_examples
